...
void swapWeight(Weight *a, Weight *b);
int sum(int a, int b);
...
void bubbleSort(Weight *data, const size_t length) {
    ...
    for(size_t i = 1; i < length; i++) {
        for(size_t j = 0; j < length-i; j++) {
            if(sum(data[j].packing, data[j].product) < sum(data[j+1].packing,data[j+1].product))
...
size_t getPivotIndex(const int *data, const size_t low, const size_t high)
{
    if(data == nullptr || low >= high)
        return 0;
    int l = data[low]; int m = data[(high+low)/2]; int h = data[high-1];
    size_t median=0;
    if((l >= m && l <= h) || (l <= m && l >= h))
        median = low;
    else if((h >= m && h <= l) || (h <= m && h >= l))
        median = high-1;
    else median = ((high+low)/2);
    return median;
}
...
{
    swapInt(&data[pivot], &data[high-1]);
    size_t insert = low;
    for(size_t i = low; i < high; i++) {
        if(data[i] >= data[high-1]) {
            swapInt(&data[i], &data[insert]);
            insert++;
        }
    }
    return insert-1;
...
int main() {
    ...
    std::cout << getPivotIndex(data, 2, 3) << endl;
    */
    /*
    Weight baliky[] = {{10, 1}, {20, 2}, {5,2}};
    bubbleSort(baliky, 3);

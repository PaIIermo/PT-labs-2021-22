...
queue<int> reverse(const list<int> & data) noexcept {
    ...
    return r; // tento riadok zmente podla zadania, je tu len kvoli kompilacii
}
...
vector<vector<int>> create2dVector(const size_t size1, const size_t size2, const int value) noexcept {
    ...
    return v0; // tento riadok zmente podla zadania, je tu len kvoli kompilacii
}
...
string connectWithSpace(const string & text, const int number) noexcept {
    string a(text + " " + std::to_string(number));
...
list<string> translate(const map<string, string> & translator, const list<string> & sentence) noexcept {
    ...
    for (const auto& key: sentence) {
        it = translator.find(key);
        if (it == translator.end()) {
            l.emplace_back("?");
        }
        else (l.emplace_back(std::string(it->second)));
    }
...
bool areParenthesisPaired(const string & mathExpression) noexcept {
    ...
    for (const auto& c: mathExpression) {
        if (c=='(' || c=='{' || c=='[' || c=='<')
            brackets.push(c);
        else if (c==')' || c=='}' || c==']' || c=='>') {
            if ((brackets.empty()
...
int main() {
    ...
    /*6std::map<string, string> translator;
    translator["Ahoj"] = "Hello";
    translator["kamarat"] = "friend";
    std::list<string> sentence { "Ahoj", "kamarat"};
    auto hullo = translate(translator, sentence);*/

    //3auto xl = create2dVector (2, 4, 8);

    /*5auto c = string("hellooooo");
    auto l = connectWithSpace(c, 45);
    std::cout << l;*/

    /*6std::string stringvalues = "125 320 a 750 333";
    std::istringstream iss (stringvalues);
    std::cout << sum(iss);*/

    /*auto c = string("(");
    bool booloo = areParenthesisPaired(c);
    std::cout << booloo;*/

    //8std::cout << execute(multiply, 20, 50) << endl;

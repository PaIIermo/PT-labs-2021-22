...
double average(const initializer_list<int> & data) {
    ...
    double l = std::accumulate(data.begin(), data.end(), 0);
    return l/data.size(); // tento riadok zmente podla zadania, je tu len kvoli kompilacii
}
...
list<int>::const_iterator findInRange200to400(const list<int> & data) noexcept {
    std::list<int>::const_iterator it;
    it = std::find_if(data.begin(), data.end(), isInRange200to400);
    return it; // tento riadok zmente podla zadania, je tu len kvoli kompilacii
}
...
list<int> add(const list<int> & data1, const list<int> & data2) noexcept {
    ...
    std::transform(data1.begin(), data1.end(), data2.begin(), l.begin(), std::plus<int>());
...
void sort1(vector<int> & data) noexcept {
    std::stable_sort(data.begin(), data.end(), std::greater<>());
}
...
void sort2(vector<int> & data) noexcept {
    std::stable_sort(data.rbegin(), data.rend(), std::less<>());
}
...
int main() {
    ...
    /*1std::initializer_list<int> s = {1, 2, 3, 4, 5};
    auto l = average(s);*/

    /*5std::list<int> s = {1,2,3,4,5};
    incrementAll(s);*/

    /*7std::list<int> s = {1,55,3,73,82};
    helpAfterExam2(s);*/

    /*8std::list<int> s = {1,2,3,4,5};
    std::list<int> l = {1,2,3,4,5};
    std::list<int> k = add(s, l);*/

    std::vector<int> s = {3,2,4,1,5};
    sort2(s);
